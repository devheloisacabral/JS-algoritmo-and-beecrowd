...
//nesse aqui cabe mais o if e o else

const prompt = require('prompt-sync')
const prompt2 = prompt()
const salario = prompt2('Digite o seu salario ')

if(salario > 0 && salario <= 2000){
    console.log('Voce está insento do imposto de renda')
} if(salario <= 2000.01 && salario <= 3000){
    const novosalario = salario - (salario * (8/100))  
    console.log('Na taxa de 8%, seu imposto de renda será ' + novosalario)
} if(salario >= 3000.01 && salario < 4000){
    const novosalario = (salario - 300) 
    const calculo = novosalario - ((18/100) * novosalario)
    const salariofinal = calculo - ((8/100) * 1000)
    console.log(`Imposto de ${salariofinal}`)
}
